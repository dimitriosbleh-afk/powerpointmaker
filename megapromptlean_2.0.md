...
# Explicit Teaching Lesson Builder Mega-Prompt v9.0
## Foundation to Year 6 | Australian Primary Schools | Visual-First | Editable | Classroom-Ready
...
- Use very large text.
- Use pictures, counters, tens frames, letter cards, objects, actions and oral response.
...
5. Cognitive load and visual clarity
...
Before finalising student-facing content, check every word against the year level.
...
Slide faces must be visual anchors, not teacher scripts.

Every slide must pass these tests:

1. Visual Anchor Test
- Is there something meaningful to look at?
- Examples: diagram, model, image, manipulative representation, chart, sentence strip, text box, card, number line, tens frame, array, map, timeline, icon, board-build space.
- If not, redesign the slide.

2. Low Text Test
- Does the slide avoid long explanations?
- If the teacher will say it, move it to SAY notes.

3. Hero Task Test
- Is the main question, prompt, number, word, image or model the biggest thing on the slide?
- If not, redesign.

4. Font Size Test
- Is all student-facing text readable from the back of the room?
- If not, enlarge or split.

5. Representation Match Test
- If the prompt says tens frame, show a tens frame.
- If it says number line, show a number line.
- If it says array, show an array.
- If it says coordinates, show a grid.
- If it says area, perimeter or volume, show the shape, boundary or 3D object.
- If it says vocabulary, show a meaningful image or graphic.

6. No Redundancy Test
- Do not put teacher narration on the slide.
- Do not write a paragraph explaining what the diagram already shows.

7. Age Test
- Would a student at this grade understand the slide quickly?
- If not, simplify.

8. Teacher Formatting Test
- Would a teacher need to resize, delete or redesign before teaching?
- If yes, fix it.
...
- 6 to 12 words where possible
- main text 44 pt or larger
- support text 30 pt or larger
- visual on almost every slide

Years 3 to 4:
- 1 idea per slide
- 1 to 2 questions or prompts
- main text 38 pt or larger
...
Do not use question numbers on slide faces unless navigation genuinely requires it.
...
Every teaching slide should use at least one of:
- photo-style visual
- simple illustration
- diagram
...
- Explain the meaning first where appropriate:
  "How many of this fraction fit into that amount?"
...
- meaningful image or graphic
- short example sentence or text context if supplied
- active practice

Active practice options:
...
If the word comes from a text:
- do not claim it appears unless the text was provided
- do not invent the sentence from the text
- use "sentence from supplied extract" placeholder if needed
...
Before generating any resource, ask:
...
- Could manipulatives do the job better?
...
- students need sorting or matching cards
- students need a collected exit ticket
- students need a self-contained extension task
- the teacher notes say distribute, hand out, print, provide, give students or use the sheet

Resources are not needed when:
- students can respond on mini-whiteboards
- students can write in workbooks
- the teacher can draw it live
- the slide is enough
- manipulatives are better
- it would be another unnecessary worksheet
...
Worksheets must look age-appropriate.
...
- avoid long lists of naked questions
- include visuals where they support the learning
- include one worked or started example if useful
- include enough room to show thinking
- keep the main task clear

Maths worksheet rules:
- Do not list 12 fraction questions with tiny answer lines.
- Give working boxes.
- Use diagrams for early questions.
- Include a concrete or visual scaffold where appropriate.
- Include fewer, better-designed problems.
- Include a separate challenge section only if there is room.
- Check every answer.

Literacy worksheet rules:
- Do not overload with written instructions.
- Use sentence frames where needed.
- Use boxes, cards, organisers and examples.
- Include visuals or text excerpts only when supplied or copyright-safe.
- Use clear writing space.
...
If no resources are needed, say:
...
Do not say "Click any resource below to open the PDF" if resources are DOCX or if no PDFs exist.
...
Teacher notes must be skimmable.
...
- connect to VTLM 2.0 only briefly if useful
...
| Age-appropriate language | Met / Partial / Missing | ... |
| Slide visual design | Met / Partial / Missing | ... |
| Cognitive load | Met / Partial / Missing | ... |
| CFU | Met / Partial / Missing | ... |
| Differentiation | Met / Partial / Missing | ... |
| Resources | Met / Partial / Missing | ... |
...
4. Run Scope Gate.
5. Choose lesson-sized target.
6. Write exactly 1 LI and exactly 3 SC.
7. Choose lesson pattern.
8. Select visual anchors and manipulatives.
9. Build slide sequence.
10. Write low-text slide faces.
11. Write teacher notes for every slide.
12. Decide if resources are needed.
13. Generate editable DOCX resources if needed.
14. Generate PDF copies only if useful.
15. Add Teacher Resources slide.
16. Run maths or content accuracy check.
17. Run visual usability check.
18. Run age-language check.
19. Run worksheet quality check.
20. Run anti-hallucination check again.
21. Run Lesson Health Check.
22. Output final files and summary.
...
- resources are PDF-only when editable was expected
...
Do not enter plan mode. Proceed with lesson creation using the provided details.

Use this prompt to generate the lesson in one pass.
