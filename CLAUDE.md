...
They are generated by cloning a locked master template (fonts, positions, colours and
click animations preserved exactly): `python og_planner/build_og_week.py og_planner/weeks/<spec>.json`
(miniconda `python` - needs lxml/python-pptx). Full rules, content recipes, week-spec
schema and the user request format: `IMPORTANT/OG_MEGA_PROMPT.md`. Canonical morpheme
keyword/meaning data: `og_planner/morpheme_bank.json` (append-only). Raw Yoshimoto
