...
// Holes Persuasive Unit (3 sessions) - Session 1: Persuasive Introduction -- Should Camp Green Lake Be Shut Down?
// Year 5/6 Literacy | Week 2
// Focus: refresh persuasive structure (introduction = hook + position + signpost) + lock in the unit's debate topic
// Anchor: Holes by Louis Sachar, Chapters 5-7 (camp conditions: heat, lack of water, dangerous work)
// Combined unit: 4 original sessions condensed into 3. This is Session 1 (was Lesson 4).
...
const FOOTER = "Persuasive Introduction | Session 1 | Year 5/6 Literacy | Holes";
const OUT_DIR = "output/Holes_Persuasive_S1_Introduction";
...
- Three sessions to plan and write a short persuasive piece
- Some of you may remember persuasive writing from earlier this year -- we will rebuild it together

DO:
- Display title slide
- Briefly preview the unit: today the introduction, then arguments with evidence and emotive punch, then counter-argument and conclusion
- Have the novel ready -- a quick re-read from Ch 5-7 to remind students of camp conditions

TEACHER NOTES:
Students met persuasive writing earlier in the year. Expect them to remember some structure but not all techniques. Today's session REFRESHES the structure and locks in the unit's topic.

WATCH FOR:
- Students who remember terms like "introduction" or "argument" -- celebrate and connect to today's slides
- Students with no recall -- reassure: "If this feels new, that is okay. We will rebuild it together"
...
TEACHER NOTES:
This is the structural backbone for the unit. Students will use it today AND next session (when they write body arguments).
...
TEACHER NOTES:
The model picks the shut-down side. Acknowledge in passing that students may pick the OTHER side. Next session they write body paragraphs using these signposted reasons.
...
EXTENDING PROMPT:
- Task: After your introduction, write your THREE reasons as topic sentences for the body paragraphs you will write next session. ("Firstly..." / "Secondly..." / "Most importantly...")
- Extra Notes: This gets confident writers ahead for next session

TEACHER NOTES:
You Do is the same topic for everyone (Camp Green Lake) -- the choice is which SIDE. Next session students write a body paragraph from the reasons they signposted today, so capture good thinking in the plan.
...
- Students with strong signposted reasons -- celebrate; flag for next session
...
- Briefly: "Next session we take your three reasons and turn them into a BODY paragraph"

TEACHER NOTES:
This sets up Session 2. Collect the strongest reasons you hear -- you may use them as examples.
...
async function build() {
  ...
  pres.author = "Claude";
  pres.title = "Holes Persuasive -- Session 1 -- Introduction Structure";
  ...
  titleSlide(
    ...
    "Session 1  |  Week 2  |  Year 5/6 Literacy",
    NOTES_TITLE
  );
  ...
  let planY = addPdfHeader(plan, "Persuasive Plan -- Camp Green Lake", {
    ...
    lessonInfo: "Session 1 | Week 2 | Year 5/6 Literacy | Holes",
    showNameDate: true,
  });

  planY = addTipBox(plan, "Today you write the INTRODUCTION only -- 3 to 4 sentences. Next session you write the body. Plan first, then write.", planY, { color: C.PRIMARY });
  ...
  planY = addBodyText(plan, "List the THREE reasons you will use in your body paragraphs next session.", planY, { fontSize: 10, italic: true });
  ...
  addPdfFooter(plan, "Session 1 | Persuasive Plan -- Page 1");

  plan.addPage();
  let planY2 = addPdfHeader(plan, "Persuasive Introduction -- Write Here", {
    color: C.PRIMARY,
    subtitle: "3 to 4 sentences only. Next session you write the body.",
    lessonInfo: "Session 1 | Week 2 | Year 5/6 Literacy",
    showNameDate: false,
  });
  ...
  addPdfFooter(plan, "Session 1 | Exit Ticket");
  ...
  addPdfFooter(mp, "Session 1 | Mentor Persuasive Introduction -- REFERENCE");

  await Promise.all([
    pres.writeFile({ fileName: `${OUT_DIR}/Holes_Persuasive_S1.pptx` }),
    writePdf(plan, PLAN_PDF_PATH),
    writePdf(mp, MENTOR_PDF_PATH),
  ]);

  console.log("PPTX written to " + `${OUT_DIR}/Holes_Persuasive_S1.pptx`);
