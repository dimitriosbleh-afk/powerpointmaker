 ...
 * - PDFs in the root of the provided folder and subfolders by default
 * - Set SEARCH_SUBFOLDERS = false below if your PDFs are only in the root folder
 */

// ── Configuration ───────────────────────────────────────────────────────────

var SEARCH_SUBFOLDERS = true;  // Set to false to search only the selected folder
...
function findBestMatch(text, pdfMap) {
  ...
  var MIN_PARTIAL_MATCH_LENGTH = 8;
  ...
  for (var i = 0; i < keys.length; i++) {
    ...
    // Text contains the PDF name. Do not match in the reverse direction:
    // generic runs such as "Answer" or "Worksheet" are often contained in
    // PDF filenames and should not become links.
    if (normalizedKey.length >= MIN_PARTIAL_MATCH_LENGTH && normalizedText.indexOf(normalizedKey) !== -1) {
