...
- **Sign card.** One bank image on a card, English meaning as the visible label, gloss as a small caption. One to four per slide; at four, all four belong to one taught set. One sign is `heroVisualSlide` with `{ type: "image", path }` and `label` set to the English meaning; two to four are `choiceSlide` options with `caption` set to the meaning (letters off when it is not a choice: `{ letters: false }`). Never a pictogram in place of a sign image: pictograms are not signs.
- **Variation slide ("Which one?").** Two bank images side by side (`<GLOSS>.jpg` and `<GLOSS>_2.jpg`) under a title like `LIBRARY - which one do you use?`, built with `choiceSlide` and `{ visual: { type: "image", path } }` options (labels `A` / `B`, never numbers). Variants are curriculum content, so build the slide when the unit document's vocabulary bank fills the Variant column for a sign the lesson teaches. Label which form the class produces and which they only need to recognise; students are not asked to learn both. Teaching point in the notes: regional variation is real, two signs can both be right, and a student's home sign goes up next to the reference once it has been checked, per the unit document's wording. Only build it when both variant images exist; a variation slide with one lookup card is just a sign card.
