...
3. Run python scripts/build_unit.py builds/manifests/<unit>.json. This builds every lesson through build_and_check.js (aborts on any gate failure) and merges the decks and PDFs into the unit folder.
...
If only one of N lessons changes later, rebuild that lesson with build_and_check.js then re-run build_unit.py with --skip-build to re-merge without rebuilding the rest.
