...
## Orton-Gillingham Decks

- Read `IMPORTANT/OG_MEGA_PROMPT.md` before authoring or editing any OG deck. OG decks use only `og_planner/build_og_week.py` and the locked `og_planner/OG_MASTER_TEMPLATE.pptx`.
- The universal OG morpheme colour key is GREEN root, YELLOW prefix, RED suffix. Enforce it on morphology review-card backgrounds, new/review morpheme-card backgrounds and Sound Bank boxes; the builder must normalise older master colours and its finished-PPTX gate must reject any mismatch.
- OG notes use the shared Glance Format: plain source text, one thought per line, left-aligned paragraphs, eight live-zone lines maximum, and no markdown. Words to Read Review is the exception: use separate cues for the whole-class read, exhaustive playful groups for rows 1-4, and everyone reading row 5 so every student reads at least two rows, followed by one retrieval question per line. The builder applies real bold to recognised labels and underlines the spelling target where it appears inside every `Sentence:` note line.
- Fixed-answer new morphology activities require an immediately following green answer slide titled `Tick it or fix it - ...`; never use `Check and fix`. New morphology grids must remain unbroken at 27 pt or larger.
- The first dictation uses the green meter and the second/trickier dictation uses yellow. Dictation reveals use green capital letters, red punctuation and bold/underlined spelling targets. Visual QA must inspect these distinctions and the notes paragraph structure.
- The builder enforces those rules against the finished PPTX, not only the JSON source. Any `WARN` or `GATE FAIL` is a blocker. Before changing the OG builder, card-note logic or sample week, run `python tests/test_og_builder_regressions.py`; it builds both the current and sample Week 1 specifications through every output gate.
